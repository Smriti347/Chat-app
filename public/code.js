(function(){
    ...
    socket.on("update", function(update){
        if(JSON.parse(JSON.stringify(update))?.username) renderMessage("other",JSON.parse(JSON.stringify(update)));
        else renderMessage("update",update);
    });

    socket.on("chat", function(message){
        console.log("on chat to other")
    ...
    function renderMessage(type, message){
        ...
        if(type == "my"){
            console.log("my",JSON.stringify(message))
            ...
            console.log("other",JSON.stringify(message))
            let e1 = document.createElement("div");
